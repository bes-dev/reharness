...
on treating ``LLM as compiler, not interpreter''~\cite{dspy,llmp,compiledai}:
intelligence is spent
...
action is a self-contained computation that emits exactly one event
(the Moore property~\cite{moore}). It is a
deliberate restriction of the Harel statechart~\cite{harel} / UML run-to-completion
hierarchical state machine (HSM)~\cite{uml,scxml}---the same
...
instance-wise array data-flow analysis~\cite{feautrier}): a stage executes once per point in its
...
framework}~\cite{kamullman}, in the lineage of Kildall's unified
lattice formulation of global program optimization~\cite{kildall}, specialised
to the lattice $L = (\powerset(K), \subseteq)$ of subsets of
...
(\textbf{must} analysis) or set union (\textbf{may} analysis). Reaching
definitions, live variables, and definite assignment are the classical
instances~\cite{dragon,nielson}.
\end{definition}
...
\begin{enumerate}[label=(\alph*),nosep]
  ...
  \item \textbf{mechanical contract}: the contract is a total computable function
        ...
        Rice's theorem~\cite{rice}, the semantic property ``this specification is a pure
...
\paragraph{LLM-as-compiler.} DSPy~\cite{dspy} compiles declarative LLM pipelines
into optimized prompts/weights; LLM+P~\cite{llmp} translates natural language into a
formal planning specification (PDDL~\cite{pddl}) delegated to a classical solver;
``Compiled AI''~\cite{compiledai} generates deterministic code at a compilation phase and executes it with
...
\emph{Statecharts}~\cite{harel} and \emph{UML} run-to-completion
semantics~\cite{uml,scxml,samek}, dropping orthogonal regions and Mealy
edge-actions (\S\ref{sec:restrictions}); states carry Moore-style
actions~\cite{moore}. The \state{parallel} construct is a fork--join task model, not
orthogonal-region concurrency.

\paragraph{Program analysis.} The analyzer is the Kam--Ullman monotone data-flow
framework~\cite{kamullman,kildall} (reaching definitions, live variables, and
definite assignment are its classical instances~\cite{dragon,nielson}) plus
transitive-closure reachability; the data-flow/workspace model is the polyhedral
iteration space (Feautrier's instance-wise array data-flow
analysis~\cite{feautrier}). Undecidability of the semantic demotability condition
(Def.~\ref{def:demote}) follows from Rice's theorem~\cite{rice}.
