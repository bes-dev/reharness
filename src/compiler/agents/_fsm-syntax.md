...
- **`config.target`** (the run's WORKING DIRECTORY) and **`config.input`** (positional args joined) are always provided — never declare them. `config.target` is where outputs go, **NOT** a user input — never read it (or `cwd`/`.`) as the thing to operate on.
- **The PRIMARY external target** (the repo/file/dir the workflow operates on) MUST be a **`required="true" positional="true"` `<arg>`**, read via `c.config.<name>`. Never an optional flag defaulting to cwd; never `config.target`.
- **Secondary parameters** (an output name, a selected field, a format) take a **concrete `default=`** (the value the demonstration showed, e.g. `default="report.md"`). An **output name / path** parameter must NEVER be `default=""` — an empty path writes nowhere; give it a real filename. (An empty default is fine only for a genuinely optional *text* value, e.g. extra context.) **Every additional INPUT the workflow reads** (one extra file or several — beyond the primary positional target) is itself a `required` input, never given a hardcoded absolute default like `default="/tmp/x/in.csv"` (that pins the workflow to the demo's machine). A concrete path from the demo is not a portable default. Make a param `required` only when there is no portable default. The command must run with just the primary target supplied.
- Read in code as `c.config.X`, in guards/`over`/`exit`/`model-expr` as `config.X`. **Every `config.X` you read must be declared.**
...
- **`ctx.data`** — in-memory **scalar** values (flags, counts, ids, parsed strings). **Only `code` and `set` states write it** (`set` via `<data>`, `code` via `c.data.x = …`). Guards/`over`/`exit`/`model-expr` read it. Agents CANNOT touch `ctx.data`. **NEVER store a filesystem path in `ctx.data`** to pass an artifact to a later stage — artifacts flow through the workspace (`c.out()` → `c.dir()`), never through data.
- **Per-stage workspace (files)** — every stage has its own output directory. At runtime the compiler derives, from the graph, **which upstream producers each stage can read**, and hands them over:
  ...
  "Who can read whom" comes from the graph edges (any ancestor producer is visible; a parallel branch's output is a per-branch list downstream). You declare none of it — so it **cannot drift**.

  **Two path namespaces:**
  - **Inter-stage artifacts** — everything a stage *produces* (a cloned repo, downloads, scratch) lives ONLY in `c.out()`, read downstream via `c.dir`/`c.dirs`.
  - **External targets** the workflow *operates on* live OUTSIDE the run — declared as `<inputs>`, reached via `c.config.<name>` (see above). The only paths pointing outside the run dir.

  **Forbidden in code states:** HARDCODED `os.tmpdir()`/`os.homedir()`/`process.env.TMPDIR`/`HOME` or absolute/`~/` literals — rejected at verify (they bind the workflow to a fixed location). A `c.config.<name>` path is a variable, not a literal, so it passes.
