...
- **A missing REQUIRED upstream producer is fail-loud — never a plausible default.** When you read an upstream output that the contract requires (an agent's `findings.json`, a prior stage's result), and it is absent/unreadable/empty, **throw** (codegen wraps it as `ERROR` → the FSM routes to its error path) — set a human-readable `c.data.error` first. Do **NOT** substitute a synthetic "success" value (e.g. defaulting a missing classification to `no_deficiency`, an empty findings list, `score=0`): that silently emits a deliverable that CONTRADICTS the real (correct) upstream judgment, and fail-loud can't see it because nothing crashed. A default value is allowed ONLY for an output the contract explicitly marks optional (then `c.warn`, per the next point). If you read a parallel producer via `c.dirs('<stage>')` and expect ≥1 branch, assert it — an empty collection from a wrong stage name or a skipped branch must throw, not fall through.
...
- **Render content ONCE (output-side need-to-know).** A code state that aggregates upstream producers (a report / summary / index) **references** their content — it does not re-render bodies an upstream stage already wrote. A full-body render helper is called in **exactly one** sink; an aggregator emits counts + an index (id/location + which stage holds the full entry) + cross-cutting synthesis. Never restate the same record through two sections of one file (e.g. a "by priority" list AND a "by category" list both with full bodies — index in one, full body in the other). Never embed a whole generated file (README, CHANGELOG, a policy doc) inside a record/finding field — emit it once as its own artifact and reference it. Re-emitting the same content through N sinks is the high-fan-in duplication the analyzer flags; it is wasted output tokens, not extra coverage.
