...
- `reharness/.cache/scratch/prd.md` — the approved intent. **The rubric.** The pipeline must implement it.
- `reharness/.cache/scratch/_compiled.md` — the whole pipeline: skeleton XML (topology + per-node `<contract>`), every agent prompt, every code-state implementation.
- `reharness/.cache/scratch/dataflow-errors.md` — if present and non-empty, deterministic use-before-def issues to resolve.
...
- agent prompts: `reharness/agents/<command>/<name>/SYSTEM.md`
...
- **If a real fix requires a topology change** (a new state, a changed transition, a missing code bridge, different wiring) — you CANNOT make it in the leaves. Do **not** force it and do **not** grind on it. Write the one-line reason to `reharness/.cache/scratch/escalate.md` and stop. The compiler will run a skeleton redesign.
...
Apply your fixes, then stop. A deterministic TypeScript compile runs after you as the objective backstop — reliable and automatic. So **do not run `tsc` / `npx` / `node` to check compilation yourself**: the target project has no local toolchain, and hunting for one only burns time. Just write code that is type-correct by construction (reason about the types), and stop.
