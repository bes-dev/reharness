...
- `reharness/.cache/scratch/prd.md` — the approved PRD (goal, behaviour, acceptance criteria, scope). **Source of truth.**
- `reharness/skills/*.md` — domain-skills (grounded context from research); use their Design considerations to choose *how* to realize stages, and reflect needed integration behaviour in contracts (the exact endpoints/auth are implemented by `fill` from the same skills).

## Output: the full skeleton at `reharness/.cache/scratch/draft-skeleton.xml`
...
2. Use the `reharness/skills/*.md` Design considerations to choose realization (retries, debate, preflight…) — only what the PRD's goal needs.
3. **Amortize reasoning into code — make a stage `code`, not `agent`, when BOTH hold:**
   - **(a) its input is structured** — every producer it reads (its graph ancestors) is a `code`/`set` stage, so the bytes have a schema your code authored (JSON, a fixed file format). Input coming from an `agent` (prose, a model's free-text answer, a raw diff, OCR) is UNstructured. **A raw user/external input (`config.<arg>` — the matter description, the control request, the ticket) is ALSO unstructured by default**: its content schema is NOT derived from the graph (it is the external channel), so the bytes are whatever the user wrote (prose, markdown, maybe JSON). The FIRST stage that reads such an input therefore fails (a) — it must be an `agent` that tolerantly extracts the fields (do NOT `JSON.parse` a user-supplied file in a `code` stage and assume a shape). Once an agent (or a `set`) has normalized it into a JSON your code authored, every downstream stage reading THAT is structured and freely demotable. (Optimization is preserved: only the single unstructured-input reader stays an agent; the mechanical core downstream is still code.)
...
6. **Render each piece of content ONCE (output-side need-to-know).** A stage that AGGREGATES many producers — a final report, a summary, an index — must **reference** their content, not restate it. Full bodies (a finding, a record, a proposed file) are materialized in exactly one stage: the one that owns them. A downstream aggregator emits **counts + an index (id/location/severity + which stage holds the detail) + cross-cutting synthesis** — never a second full copy. The same producer fanned out to several full-render sinks is duplicated output (the analyzer's high-fan-in signal). This is the dual of input need-to-know: as a leaf must not be injected with what it doesn't need, a sink must not re-emit what an upstream stage already materialized. (Make the `<contract>` say "index/summarize, do not restate the per-stage bodies".)
...
- **`<inputs>` — the ONE thing you declare about external data** (it comes from the user, not the graph). Declare it from the PRD's Inputs; full syntax is in the appended reference. Two rules matter at design time:
  - The **primary target** the workflow operates on (a repo/file/dir) is a `required` `positional` `<arg>` — never `config.target` (the working dir), never cwd.
  - Secondary parameters take a `default=` (the PRD's value); the command must run with just the primary target supplied.
- **`ctx.data`** — scalars only; written only by `code`/`set`; read by guards/`over`/`exit`/`model-expr`. A `<contract>` must NEVER route a file/artifact through it (artifacts flow through the workspace).
- **Per-stage workspace** — derived from the graph; each stage gets `c.out()` and its upstream producers' dirs (`c.dir('<stage>')`, or `c.dirs('<stage>')` for a parallel branch). You declare none of it — no paths. A contract refers to data by **what it is**, not where it lives. (External targets are the exception: a stage may read/write `config.<name>`.) Structural duty: when a guard needs an agent's result, add a `code` bridge (point 5).
...
- Edit **only** `reharness/.cache/scratch/draft-skeleton.xml`.
