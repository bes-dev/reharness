...
The skeleton has been validated and codegen has produced stubs. You run **in parallel** with `fill_prompts_lib` which fills code-state implementations. **You only edit `reharness/agents/<command>/<name>/SYSTEM.md` files** (each agent lives in its own directory). Don't touch lib code or anything else.
...
- `reharness/agents/<command>/<name>/SYSTEM.md` — agent master-prompt stubs (look for `<!-- TODO`)
- `reharness/skills/*.md` — domain-skills from research. When a leaf's task is domain-specific, ground its prompt in the relevant skill (accurate domain facts/format) rather than training memory; the runtime also attaches the skill to the leaf.
- `reharness/.cache/feedback/` — accumulated feedback from prior revisions
- `reharness/.cache/scratch/verify-errors.md` — if present, fix listed errors related to agent prompts

## What to write

For each **agent state** (`reharness/agents/<command>/<name>/SYSTEM.md`) that still contains `<!-- TODO`:
