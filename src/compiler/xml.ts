...
import { parseGuard, formatGuard } from "./expr.js";
...
function parseState(raw: any): SkeletonState {
  ...
  if (raw["@_timeout"] && raw["@_type"] !== "wait") st.timeout = raw["@_timeout"]; // wait owns @_timeout (→ waitTimeout); don't ALSO set the universal field (would double-emit the attr + a duplicate timeoutMs)
