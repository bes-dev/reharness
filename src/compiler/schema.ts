/** Skeleton ids reserved by reharness itself: the internal meta-pipeline (`generate`) and every built-in CLI verb
 *  (the `VERBS` table in cli.ts) — a compiled command must not shadow a toolchain command in the shared `reharness
 *  <name>` namespace. Keep in sync with cli.ts VERBS. */
export const RESERVED_IDS = new Set(["generate", "compile", "amend", "evolve", "graph"]);
