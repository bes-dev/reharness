...
export function lintSkeleton(sk: Skeleton): string[] {
  ...
  // A hardcoded absolute SYSTEM path baked into an arg default leaks a machine-specific location from the
  // demonstration — the workflow then only runs on that exact path. (An env-rooted `~/…` default is fine: it
  // resolves per-user at the command boundary, so it stays portable.) A path with no portable default should be
  // a required input, not a pinned literal.
  for (const a of sk.inputs || []) {
    if (typeof a.default === "string" && /^\/(?:tmp|var|Users|home|private|opt|etc|root)(?:\/|$)/.test(a.default)) {
      errors.push(`Arg '${a.name}' default '${a.default}' is a hardcoded absolute path from the demonstration — it pins the workflow to one machine-specific location. A path input has no portable default: make it required="true" (a positional arg the user supplies), or, for an OUTPUT written under the run, use a plain relative filename.`);
    }
  }
  ...
  if (sk.id && RESERVED_IDS.has(sk.id)) {
    errors.push(`Skeleton id '${sk.id}' is reserved — it names a built-in reharness command (${[...RESERVED_IDS].join(", ")}).`);
  }
