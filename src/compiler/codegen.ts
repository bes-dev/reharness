...
/** Single-file consolidated view of the entire pipeline (skeleton + lib + agents + scope).
 *  Used by the `review` and `redesign` agents so they read one file instead of 17+. */
function emitCompiled(sk: Skeleton, reharnessDir: string): void {
  const genDir = resolve(reharnessDir, "generate");
  if (!existsSync(genDir)) return; // no scope yet — codegen called outside /generate flow
  const compiledPath = resolve(genDir, "_compiled.md");
  const skeletonPath = resolve(reharnessDir, "skeletons", `${sk.id}.xml`);
  const libPath = resolve(reharnessDir, "lib", `${sk.id}-states.ts`);
  const agentsDir = resolve(reharnessDir, "agents");
  const scopePath = resolve(genDir, "scope.md");

  const parts: string[] = [];
  parts.push(`# Compiled Pipeline: ${sk.id}\n`);
  parts.push(`> Auto-generated by codegen. Single-file view of the entire pipeline for review/redesign agents — read this file instead of opening skeleton + lib + every agent prompt + scope separately.\n`);

  if (existsSync(scopePath)) {
    parts.push(`## Scope (technical spec)\n`);
    parts.push("```markdown\n" + readFileSync(scopePath, "utf-8").trimEnd() + "\n```\n");
  }

  if (existsSync(skeletonPath)) {
    parts.push(`## Skeleton XML\n`);
